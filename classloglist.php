...
$s = new StudentForm;
$dc = new Database;
$response = $s->list_classLog();
$total = count($response); 

$query = "SELECT st.`StudentID`, `FirstName`, `LastName`, `Gender`, `City`, `Licence`,  `logClassID`, `InClassDate`, cl.`Location`, `Hours`, `Instructor` FROM class_log as cl left join `student_information` as st on `st`.`StudentID`= `cl`.`StudentID`  where cl.`isActive`=1 and `st`.`LastName` IS NOT NULL  ORDER BY `st`.`LastName`  asc";
$records_per_page = 50;
...
<div class="row">
    <div class="col-md-12">
<form action="searchclasslog.php" method="post" enctype="multipart/form-data">
...
{
	 $rowcount++;
$dateformt = explode("/",$val['InClassDate']);	
$count++;
?>
  <tr align="center">
    ...
    <td><?php $arr = array(); 
					...
					?></td>
    <td><a href="edit_inclass.php?id=<?php echo $val['logClassID']?>" border="0" ><img src="images/edit.gif" border="0" /></a></td>
    <td><a href="action_page.php?mode=delete&form=classLog&id=<?php echo ($val['logClassID']);?>" onclick="return checkmode(3)" >
   ...
   <div class="col-sm-5  text-left"><button class="btn btn-default btn-lg active" onclick="javascript: checkAll(this, <?php echo $total; ?> )">All</button> &nbsp; 
								...
								<a class="btn btn-default btn-lg active" href="javascript:void(0);" 
									...
									<img src="images/deleteall.png" border=0/>&nbsp;Delete All</a> &nbsp;&nbsp;&nbsp; 
                                     <a href="trash_classloglist.php" class="btn btn-default btn-lg active"  >Trash</a></div>  
  		<div class=""><a  href="javascript:void(0);" 
        onclick="javascript: doCheckBoxAction(this, <?php echo $total; ?>, 'xcel/classxcelsheet.php?tableid=incLectrID&tablename=inc_lectr_log&ids=',0);" 
