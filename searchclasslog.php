...
OR '$keyw' LIKE  Concat(Concat('%',LastName),'%') OR cl.Location LIKE '%$keyw%' OR Instructor LIKE '%$keyw%' ";
 ...
 $query = "SELECT st.`StudentID`, `FirstName`, `LastName`, `Gender`, `City`, `Licence`,  `logClassID`, `InClassDate`, cl.`Location`, `Hours`, `Instructor` FROM class_log as cl left join `student_information` as st on `st`.`StudentID`= `cl`.`StudentID`  where cl.`isActive`=1 $condition  ORDER BY `cl`.`logClassID`  DESC";
$records_per_page = 50;
...
<div class="row">
    <div class="col-md-12">
<form action="searchclasslog.php" method="get" enctype="multipart/form-data">
