...
//$col = $_REQUEST['searchby'];  OR FirstName LIKE '%$keyw%' OR Location LIKE '%$keyw%' '$keyw' LIKE Concat(Concat('%',FirstName),'%') 
//OR '$keyw' LIKE  Concat(Concat('%',LastName),'%')
$keyw = mysql_real_escape_string(trim($_REQUEST['search']));
$condition = " '$keyw' LIKE Concat(Concat('%',FirstName),'%') 
OR '$keyw' LIKE  Concat(Concat('%',LastName),'%')  OR Location LIKE '%$keyw%' OR Address LIKE '%$keyw%' OR City LIKE '%$keyw%' ";
 $query = "SELECT * FROM `student_information` where  $condition and `isActive`=1 ORDER BY `student_information`.`StudentID`  DESC";
