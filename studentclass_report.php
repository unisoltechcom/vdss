...
$classhr =  CLASSHOURS;
$locationQ='';
if (@$_REQUEST['Location']!='') { $locationQ = "and `si`.`Location` like '".$_REQUEST['Location']."' "; }
$querysort = '';
if(@$_REQUEST['sorby']=='hours') {	
		$querysort = "GROUP BY cl.StudentID having sum(cl.Hours) = $classhr order by `si`.`LastName` ASC";
		$query = "SELECT `si`.`StudentID`,`si`.`FirstName`, `si`.`LastName`, `si`.`Gender`, `si`.`City`, `si`.`Location` , `si`.`Phone`, `si`.`Owing`, `si`.`School` , `si`.`LicenceClass` , `cl`.`InClassDate` , cl.Instructor ,sum(cl.Hours) as Hours,sum(cl.Hours) as total  FROM `student_information` as si left join  `class_log` as cl on `cl`.`StudentID` = `si`.`StudentID`  where `cl`.`isActive`=1 ".$Inscond."  ".@$_SESSION['cdatebetween']." ".$locationQ." ".$querysort."  ";
	} else if (@$_REQUEST['sorby']=='timepending') {
		$querysort = "GROUP BY cl.StudentID having sum(cl.Hours) < $classhr order by `si`.`LastName` ASC";
		$query = "SELECT `si`.`StudentID`,`si`.`FirstName`, `si`.`LastName`, `si`.`Gender`, `si`.`City`, `si`.`Location` , `si`.`Phone`, `si`.`Owing`, `si`.`School` , `si`.`LicenceClass` , `cl`.`InClassDate` , cl.Instructor ,sum(cl.Hours) as Hours,sum(cl.Hours) as total FROM `student_information` as si left join  `class_log` as cl on `cl`.`StudentID` = `si`.`StudentID`  where `cl`.`isActive`=1  ".@$_SESSION['cdatebetween']." ".$locationQ." ".$querysort."  ";
	
	} else if (@$_REQUEST['sorby']=='Owing') {
		$querysort = "and `si`.`Owing` > 0 group by `cl`.`StudentID` order by `si`.`LastName` ASC";
		$query = "SELECT `si`.`StudentID`,`si`.`FirstName`, `si`.`LastName`, `si`.`Gender`, `si`.`City`, `cl`.`Location` , `si`.`Phone`, `si`.`Owing`, `si`.`School` , `si`.`LicenceClass` , `cl`.`InClassDate` , cl.Instructor ,sum(cl.Hours) as Hours,sum(cl.Hours) as total  FROM `student_information` as si left join  `class_log` as cl on `cl`.`StudentID` = `si`.`StudentID`  where  `si`.`isActive`=1  ".$Inscond."   ".$_SESSION['cdatebetween']." ".$locationQ." ".$querysort."  ";
	
	} else if (@$_REQUEST['Location']!='') {
		$querysort = "and `si`.`Location` like '".$_REQUEST['Location']."' order by `si`.`LastName` ASC";
		$query = "SELECT `si`.`StudentID`,`si`.`FirstName`, `si`.`LastName`, `si`.`Gender`, `si`.`City`, `cl`.`Location` , `si`.`Phone`, `si`.`Owing`, `si`.`School` , `si`.`LicenceClass` , `cl`.`InClassDate` , cl.Instructor ,`cl`.`Hours`  FROM `student_information` as si left join  `class_log` as cl on `cl`.`StudentID` = `si`.`StudentID`  where  `si`.`isActive`=1  ".$Inscond."    ".$_SESSION['cdatebetween']." ".$querysort."  ";
	
	}  else   {
		$querysort = "order by `si`.`LastName` ASC";
		$query = "SELECT cl.logClassID,`si`.`StudentID`,`si`.`FirstName`, `si`.`LastName`, `si`.`Gender`, `si`.`City`, `cl`.`Location` , `si`.`Phone`, `si`.`Owing`, `si`.`School` , `si`.`LicenceClass` , `cl`.`InClassDate` , cl.Instructor ,`cl`.`Hours`  FROM `student_information` as si left join  `class_log` as cl on `cl`.`StudentID` = `si`.`StudentID`  where `cl`.`isActive`=1  ".$Inscond."    ".@$_SESSION['cdatebetween']." ".$querysort."  ";
	}
//print $query; STR_TO_DATE(InClassDate, '%d/%m/%Y')
...
<div class="wrapper">
  ...
  <div class="student">List of students who are in class by date</div>
  ...
  <div class="row">
      <div class="col-md-12 reportview"> 
        ...
        <form action="" method="get" class="instructor studentdriving classstudentlogs"> 
        <div class="col-sm-3 no-padding">
          <div class="form-group  ">
            <label class="control-label no-padding-right no-padding-left col-sm-3" >Instructor:</label>
       ...
       <div class="col-sm-3 no-padding">
          <div class="form-group">
            <label class="control-label col-sm-3 no-padding-right" >Location:</label>
            <div class="col-sm-9 inputwidth-96 no-padding-right no-padding-left">
              ...
              <script>jQuery('#Location  option[value="<?=$_REQUEST['Location']?>"]').prop("selected", true);</script>
...
<td align="left" class="text-success"><strong><?=$pager["total_records_found_text"]?></strong></td> 
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr align="center">
    ...
    <?php if( @$_REQUEST['sorby']=='date' ) { ?><th scope="col" align="center"> Hours</th><?php } else { ?>   
    ...
    <th scope="col" align="center">InClassDate</th>
    <th width="2%">&nbsp;</th>
    <th width="2%">&nbsp;</th>
    <th width="2%">&nbsp;</th>  
  </tr>
...
?>
  <tr align="center">
    <td><input type="checkbox" id="chk<?php echo $row_count; ?>" name="chkCustomer<?php echo $row_count; ?>" 
								  value="<?php echo $val['logClassID']; ?>" onclick="selectDeselectAll(this, <?php echo $total; ?>, 2);" /></td>
    <td><?php echo $val['StudentID']; ?><?php //=$rowcount?></td>
    <td><?php //$val['Instructor']?>
	<?php $arr = array(); 
					$arr = explode(" ",$val['Instructor']);
    ...
    <td><?=$val['Phone']?></td>
    <td><?=$val['Owing']?></td>
     <?php if( @$_REQUEST['sorby']=='date' ) { ?><td><?=$val['Hours']?></td> <?php } else { ?>   
    <td><?=$val['Hours']?></td> 
    <td><?php if(@$val['total']) { print ($classhr-@$val['total']); } else { print '0'; }  ?></td> 
    <?php } ?> 
    <td><?=$val['School']?></td>
    <td><?=$val['Location']?></td>
    <td><?php print dateformat($val['InClassDate']);?></td>         
    <td><a href="view_student.php?id=<?php echo $val['StudentID']?>" border="0" ><img src="images/view.png" border="0" /></a></td>
    <td><a href="edit_inclass.php?id=<?php echo $val['StudentID']?>" border="0" ><img src="images/edit.gif" border="0" /></a></td>
    <td><a href="action_page.php?mode=delete&form=classLog&id=<?php echo ($val['StudentID']);?>" onclick="return checkmode(3)" >
  ...
  <div class="col-sm-12 text-right">
   <div class="col-sm-5  text-left"><span class="btn btn-default btn-lg active" onclick="javascript: checkAll(this, <?php echo $total; ?> )">All</span> &nbsp; 
								...
								<a class="btn btn-default btn-lg active" href="javascript:void(0);" 
								    onclick="javascript: doCheckBoxAction(this, <?php echo $total; ?>, 'action_page.php?mode=selectedDelete&form=classLog&id=',4);" >
        ...
        onclick="javascript: doCheckBoxAction(this, <?php echo $total; ?>, 'xcel/classreportxcelsheet.php?tableid=incLectrID&tablename=inc_lectr_log&ids=',0);" 
