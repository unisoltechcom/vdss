...
$query = "SELECT st.`StudentID`, `FirstName`, `LastName`, `Gender`, `City`, `Licence`,  `logCarID`, `InCarDate`, cl.`Location`, `Hours`, `Instructor` FROM car_log as cl left join `student_information` as st on `st`.`StudentID`= `cl`.`StudentID`    where cl.`isActive`=1 and `st`.`LastName` IS NOT NULL  ORDER BY `st`.`LastName`  asc";
$records_per_page = 50;
...
<div class="row">
    <div class="col-md-12">
<form action="searchcarlog.php" method="post" enctype="multipart/form-data">
...
<br />
  ...
  <div class="col-sm-12 text-right">
   <div class="col-sm-5  text-left"><button class="btn btn-default btn-lg active" onclick="javascript: checkAll(this, <?php echo $total; ?> )">All</button> &nbsp; 
        ...
        onclick="javascript: doCheckBoxAction(this, <?php echo $total; ?>, 'xcel/carxcelsheet.php?tableid=incLectrID&tablename=inc_lectr_log&ids=',0);" 
