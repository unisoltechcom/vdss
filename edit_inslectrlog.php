...
$dc = new Database;
$inlogis = $_REQUEST['id'];
$query = "SELECT * FROM `inc_lectr_log` as inc where inc.`isActive`=1 and `inc`.`incLectrID`=$inlogis ORDER BY `inc`.`incLectrID` DESC";
$record = $dc->execute_query($query);
$response = @mysqli_fetch_array($record);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	$incLectrID = $_POST['incLectrID'];
	unset($_POST['incLectrID']);
	$data = $_POST;
	$table='inc_lectr_log';
	$where = "incLectrID = '$incLectrID'";
  ...
  <form class="form-horizontal" name="myForm"  action="" method="post" onsubmit="return validateForm(this);">
    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6">
		  ...
		  $responses = $s->list_instructor();
		  foreach($responses as $val)
		  {  ?>               
            <option value="<?=$val['Ins_id']?>"><?=$val['Ins_First_Name'].' '.$val['Ins_Last_Name']?></option>
                <?php }?>
              </select>
              <script>jQuery('#Ins_id  option[value="<?=$response['Ins_id']?>"]').prop("selected", true);</script>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-sm-6">
          <div class="form-group">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control"  name="schoolName" value="<?=$response['schoolName']?>" >
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8">
              <div class="dropdown">
                ...
                <script>jQuery('#course  option[value="<?=$response['course']?>"]').prop("selected", true);</script>
              </div>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-sm-6">
          <div class="form-group">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control" name="lessonDate" value="<?=$response['lessonDate']?>" placeholder="dd/mm/yy" id = "datepicker-13">
            </div>
          </div>
        </div>
        <div class="col-md-6 col-sm-6">
          <div class="form-group">
            ...
            <div class="col-sm-8">
              <select class="form-control" id="hours" name="hours">
               ...
               <script>jQuery('#hours  option[value="<?=$response['hours']?>"]').prop("selected", true);</script>
            </div>
          </div>
        </div>
     ...
     <div class="col-md-6 col-sm-6">
          <div class="form-group">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control"  name="sub" value="<?=$response['sub']?>">
            </div>
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <div class="form-group">
          ...
          <input type="hidden" name="incLectrID" value="<?=$response['incLectrID']?>"  />
